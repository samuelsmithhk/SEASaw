...
class IndexDotHTMLAwareStaticFileHandlerL(web.StaticFileHandler):
    def parse_url_path(self, url_path):
        if not url_path or url_path.endswith('/'):
            url_path += 'index_lin.html'
        return super(IndexDotHTMLAwareStaticFileHandlerL, self).parse_url_path(url_path)

class SearchHandlerL(RequestHandler):
    @gen.coroutine
    def get(self):
        lists = []
        infile0 = open('./pickleFiles/InvertedIndex0.pickle', 'rb')
        ...
        doc_ids = list(doc_vector_dict.keys())
        if len(doc_ids) == 0:
            self.render("template_lin.html", searchbar=searchbar, results="<center><h2> Results not found! Please try another query! <h2></center>")
            return
        doc_matrix = np.zeros((len(doc_vector_dict), len(query_terms)))
        for doc_ix, doc_id in enumerate(doc_ids):
            doc_matrix[doc_ix][:] = doc_vector_dict[doc_id][:]
        ...
        self.render("template_lin.html", searchbar=searchbar, results=res_row)
